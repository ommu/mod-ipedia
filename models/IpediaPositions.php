...
class IpediaPositions extends \app\components\ActiveRecord
{
	 ...
	 */
	public function getSkills($count=true, $publish=1)
	{
		if($count == true) {
	...
	public function init()
	{
		...
		$this->templateColumns['skills'] = [
			...
			'filter' => false,
			'value' => function($model, $key, $index, $column) {
				return Html::a($model->skills, ['skill/manage', 'position'=>$model->primaryKey, 'publish'=>1], ['title'=>Yii::t('app', '{count} skills', ['count'=>$model->skills])]);
			},
			'contentOptions' => ['class'=>'center'],
			'format' => 'html',
		];
