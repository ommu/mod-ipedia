...
class PositionskillController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = IpediaPositionSkill::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('IpediaPositionSkill', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Ipedia Position Skills');
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/o/position_skill/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/o/position_skill/front_view',array(
			'model'=>$model,
		));
		/*
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['IpediaPositionSkill'])) {
			$model->attributes=$_POST['IpediaPositionSkill'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaPositionSkill success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaPositionSkill success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Ipedia Position Skills');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/o/position_skill/admin_add',array(
			'model'=>$model,
		));
	}
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['IpediaPositionSkill'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaPositionSkill success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Ipedia Position Skills');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/o/position_skill/admin_edit',array(
			'model'=>$model,
		));
	}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/o/position_skill/admin_headline');
		}
	}
