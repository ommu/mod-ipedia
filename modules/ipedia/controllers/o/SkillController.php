...
class SkillController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = IpediaSkills::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('IpediaSkills', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Ipedia Skills');
	...
	{
		$model=new IpediaSkills;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['IpediaSkills'])) {
			$model->attributes=$_POST['IpediaSkills'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-ipedia-skills',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaSkills success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaSkills success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['IpediaSkills'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-ipedia-skills',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaSkills success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaSkills success updated.'));
					//$this->redirect(array('view','id'=>$model->skill_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				IpediaSkills::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				IpediaSkills::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-ipedia-skills',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaSkills success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'IpediaSkills Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = IpediaSkills::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ipedia-skills-form') {
