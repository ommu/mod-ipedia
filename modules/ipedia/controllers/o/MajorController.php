...
class MajorController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = IpediaMajors::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('IpediaMajors', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Ipedia Majors');
	...
	{
		$model=new IpediaMajors;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['IpediaMajors'])) {
			$model->attributes=$_POST['IpediaMajors'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-ipedia-majors',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaMajors success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaMajors success created.'));
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['IpediaMajors'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-ipedia-majors',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaMajors success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaMajors success updated.'));
					//$this->redirect(array('view','id'=>$model->major_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				IpediaMajors::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				IpediaMajors::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-ipedia-majors',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaMajors success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'IpediaMajors Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = IpediaMajors::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ipedia-majors-form') {
