...
class UniversitymajorController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = IpediaUniversityMajor::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('IpediaUniversityMajor', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'Ipedia University Majors');
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/o/university_major/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/o/university_major/front_view',array(
			'model'=>$model,
		));
		/*
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['IpediaUniversityMajor'])) {
			$model->attributes=$_POST['IpediaUniversityMajor'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'IpediaUniversityMajor success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaUniversityMajor success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Ipedia University Majors');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/o/university_major/admin_add',array(
			'model'=>$model,
		));
	}
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['IpediaUniversityMajor'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'IpediaUniversityMajor success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Ipedia University Majors');
		$this->pageDescription = '';
		$this->pageMeta = '';
		$this->render('/o/university_major/admin_edit',array(
			'model'=>$model,
		));
	}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/o/university_major/admin_headline');
		}
	}
